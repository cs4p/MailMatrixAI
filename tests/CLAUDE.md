...
- **Flask tests**: use `client` fixture from `test_app.py` which patches `RULES_PATH` and `ENV_PATH` to tmp paths so tests never touch real files (it also resets the module-level inbox-count cache).
